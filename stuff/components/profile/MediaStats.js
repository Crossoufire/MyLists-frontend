...
import {capitalize, getStatusColor} from "@/stuff/utils/functions";
...
const MediaStats = ({ user, media }) => (
    <div className="d-flex flex-column gap-4 m-t-15">
                ...
                {media.status_count.map(s =>
                    <React.Fragment key={`${s.status}-${media.media_type}`}>
                        <Col className="col-4 one-line-ellipsis">
                            <Link href={`/list/${media.media_type}/${user.username}?status=${s.status}`}
        ...
        <div>
            <Link className="text-light fw-5 fs-18" href={`/list/${media.media_type}/${user.username}?status=Favorite`}>
            ...
            {media.total_favorites === 0 ?
                ...
                <div className="d-flex flex-wrap justify-content-start gap-2 m-t-5">
                    {media.favorites.map(m =>
                        <Link key={m.media_name} href={`/details/${media.media_type}/${m.media_id}`}>
        ...
        <div className="fw-5 fs-18">
            <Link className="text-light" href={`/stats/${media.media_type}/${user.username}`}>
