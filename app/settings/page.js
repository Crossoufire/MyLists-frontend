...
import {maxWidthSettings} from "@/stuff/utils/constants";
import AddTooltip from "@/stuff/components/primitives/AddTooltip";
...
const SettingsForm = () => {
    ...
    const apiCall = async (formData) => {
        ...
        try {
            ...
            response = await fetch(`${process.env.NEXT_PUBLIC_BASE_API_URL}/api/update_settings`, body);
        }
        catch (error) {
            response = {
                ok: false,
