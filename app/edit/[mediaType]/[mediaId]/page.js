...
const MediaEditPage = ( {params} ) => {
    const api = useApi();
    const flash = useFlash();
    const router = useRouter();
    const { register, handleSubmit } = useForm();
    const { apiData, loading, error } = useFetchData(`/details/form/${params.mediaType}/${params.mediaId}`);

    const onSubmit = async (data) => {
        const response = await api.post(`/details/form/${params.mediaType}/${params.mediaId}`, data)
        ...
        return router.push(`/details/${params.mediaType}/${params.mediaId}`);
    };
